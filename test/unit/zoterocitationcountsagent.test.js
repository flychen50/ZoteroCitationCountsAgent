...
describe('ZoteroCitationCountsAgent', function() {
  ...
  beforeEach(function() {
    ...
    if (!zccCode) {
      zccCode = fs.readFileSync(path.join(__dirname, '../../src/zoterocitationcountsagent.js'), 'utf-8');
    }
    
    // Execute the script content, making ZoteroCitationCountsAgent available globally
    ...
    // Ensure that ZoteroCitationCountsAgent.getPref is bound to the Zotero.Prefs.get mock for these tests
    // This is necessary because the original script uses 'this.getPref' internally for _nasaadsUrl
    // and 'this' inside _nasaadsUrl refers to ZoteroCitationCountsAgent.
    // We need to ensure that ZoteroCitationCountsAgent.getPref calls our stubbed Zotero.Prefs.get.
    // The ZoteroCitationCountsAgent object is defined as an object literal, and its methods
    // like getPref are defined within that literal. When these methods are called,
    // 'this' correctly refers to ZoteroCitationCountsAgent.
    // The 'getPref' method in ZoteroCitationCountsAgent itself calls Zotero.Prefs.get,
    // which we have stubbed. So, direct assignment as below is not strictly needed
    // if the script structure ensures 'this' is ZoteroCitationCountsAgent.
    // However, explicitly binding or ensuring the methods use the mocked global.Zotero
    // can be a good safeguard.
    // In this case, ZoteroCitationCountsAgent.getPref will use global.Zotero.Prefs.get due to lexical scoping
  ...
  afterEach(function() {
    ...
    delete global.ZoteroCitationCountsAgent; // Clean up the global scope
  });

  describe('_nasaadsUrl', function() {
    it('should construct the correct URL with API key for DOI', function() {
      global.Zotero.Prefs.get.withArgs('extensions.zoterocitationcountsagent.nasaadsApiKey', true).returns('TEST_API_KEY');
      
      const id = '10.1000/xyz123';
      const type = 'doi';
      const actualUrl = global.ZoteroCitationCountsAgent._nasaadsUrl(id, type);
    ...
    it('should construct the correct URL with API key for arXiv', function() {
      global.Zotero.Prefs.get.withArgs('extensions.zoterocitationcountsagent.nasaadsApiKey', true).returns('TEST_API_KEY_ARXIV');
    ...
    it('should use an empty string if API key is not set', function() {
      global.Zotero.Prefs.get.withArgs('extensions.zoterocitationcountsagent.nasaadsApiKey', true).returns('');
  ...
  describe('getPref', function() {
    it('should call Zotero.Prefs.get with the correct preference key', function() {
      // The ZoteroCitationCountsAgent object is globally available.
      global.ZoteroCitationCountsAgent.getPref('myTestPref');
      expect(global.Zotero.Prefs.get.calledOnceWith('extensions.zoterocitationcountsagent.myTestPref', true)).to.be.true;
    });

    it('should return the value from Zotero.Prefs.get', function() {
      global.Zotero.Prefs.get.withArgs('extensions.zoterocitationcountsagent.anotherPref', true).returns('expectedValue');
      const val = global.ZoteroCitationCountsAgent.getPref('anotherPref');
  ...
  describe('_sendRequest', function() {
    ...
    it('should throw zoterocitationcountsagent-progresswindow-error-nasaads-apikey error for NASA ADS 401 response', async function() {
      ...
      try {
        await global.ZoteroCitationCountsAgent._sendRequest(nasaAdsUrl, mockCallback);
      } catch (e) {
        actualError = e;
      }
      expect(actualError).to.be.an('Error');
      expect(actualError.message).to.equal('zoterocitationcountsagent-progresswindow-error-nasaads-apikey');
      expect(global.Zotero.debug.calledWith(sinon.match(/NASA ADS API key error/))).to.be.true;
    });

    it('should throw zoterocitationcountsagent-progresswindow-error-nasaads-apikey error for NASA ADS 403 response', async function() {
      global.fetch.resolves({
        ok: false,
        status: 403,
        url: nasaAdsUrl
    ...
    it('should throw zoterocitationcountsagent-progresswindow-error-bad-api-response error for other NASA ADS errors (e.g., 500)', async function() {
      global.fetch.resolves({
        ok: false,
        status: 500,
        url: nasaAdsUrl
      ...
      expect(actualError).to.be.an('Error');
      expect(actualError.message).to.equal('zoterocitationcountsagent-progresswindow-error-bad-api-response');
      expect(global.Zotero.debug.calledWith(sinon.match(/Bad API response for/))).to.be.true;
    });

    it('should throw zoterocitationcountsagent-progresswindow-error-bad-api-response error for non-NASA ADS API errors (e.g., 404)', async function() {
      global.fetch.resolves({
        ok: false,
        status: 404,
        url: otherApiUrl
      ...
      let actualError = null;
      try {
        await global.ZoteroCitationCountsAgent._sendRequest(otherApiUrl, mockCallback);
      } catch (e) {
        actualError = e;
      }
      expect(actualError).to.be.an('Error');
    ...
    it('should throw zoterocitationcountsagent-progresswindow-error-bad-api-response error for network failures', async function() {
    ...
    it('should return count for successful response and valid count', async function() {
      ...
      const count = await global.ZoteroCitationCountsAgent._sendRequest(otherApiUrl, mockCallback);
      expect(count).to.equal(123);
      expect(mockCallback.calledWith(mockResponseData)).to.be.true;
    });

    it('should throw zoterocitationcountsagent-progresswindow-error-no-citation-count error for successful response but invalid count (string)', async function() {
      ...
      expect(actualError).to.be.an('Error');
      expect(actualError.message).to.equal('zoterocitationcountsagent-progresswindow-error-no-citation-count');
      expect(global.Zotero.debug.calledWith(sinon.match(/Error processing API response/))).to.be.true;
    });
    
    it('should throw zoterocitationcountsagent-progresswindow-error-no-citation-count error for successful response but invalid count (negative)', async function() {
    ...
    it('should throw zoterocitationcountsagent-progresswindow-error-no-citation-count error for successful response but malformed JSON', async function() {
      global.fetch.resolves({
        ok: true,
        status: 200,
      ...
      try {
        ...
        actualError = e;
          ...
          it('should send Authorization header for NASA ADS requests', async function() {
            // Arrange
            global.Zotero.Prefs.get.withArgs('extensions.zoterocitationcountsagent.nasaadsApiKey', true).returns('MY_NASA_KEY');
        ...
        json: sinon.stub().resolves(mockResponseData)
            ...
            // Act
            await global.ZoteroCitationCountsAgent._sendRequest(nasaUrl, callback);
