...
describe('Crossref Integration Tests', function() {
  let sandbox;
  let mockZotero;
  let mockItem;
  let crossrefAPI;
  ...
  beforeEach(async () => {
    ...
    crossrefAPI = global.ZoteroCitationCounts.APIs.find( // Use global.ZoteroCitationCounts
      (api) => api.key === "crossref"
    );
    expect(crossrefAPI).to.exist;
  });
  ...
  describe("Crossref API Tests", () => {
    it("Scenario 1: Successful fetch and update via DOI", async () => {
      ...
      await global.ZoteroCitationCounts.updateItems([mockItem], crossrefAPI); // Use global.ZoteroCitationCounts

      const expectedUrl =
        "https://api.crossref.org/works/10.1000%2Fxyz123/transform/application/vnd.citationstyles.csl+json";
      expect(global.fetch.calledOnceWith(expectedUrl)).to.be.true;
      expect(mockItem.setField.calledOnceWith("extra", `123 citations (Crossref/DOI) [${today}]`)).to.be.true;
    ...
    it("Scenario 2: Error handling for invalid DOI", async () => {
      ...
      const expectedUrl =
        "https://api.crossref.org/works/10.1000%2Finvalid/transform/application/vnd.citationstyles.csl+json";
      expect(global.fetch.calledOnceWith(expectedUrl)).to.be.true;
      ...
      expect(formatValueStub.calledWith("citationcounts-progresswindow-error-bad-api-response", { api: "Crossref" })).to.be.true;
    });

    it("Scenario 3: No citation count found scenario", async () => {
      mockItem = createMockItem(sandbox, { DOI: "10.1000/nocount" }); // Pass sandbox
      mockZotero.getActiveZoteroPane().getSelectedItems.returns([mockItem]);
      ...
      const expectedUrl =
        "https://api.crossref.org/works/10.1000%2Fnocount/transform/application/vnd.citationstyles.csl+json";
      expect(global.fetch.calledOnceWith(expectedUrl)).to.be.true;
      expect(mockItem.setField.calledOnceWith("extra", `0 citations (Crossref/DOI) [${today}]`)).to.be.true;
      expect(mockItem.saveTx.calledOnce).to.be.true;
    });
  });
});
